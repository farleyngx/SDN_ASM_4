...
export default function DashboardPage() {
  ...
  const { questions, loading: loadingQuestions } = useSelector((state: any) => state.manager);
  ...
  // Question Creation Form State (for logged-in users)
    ...
    <div className="position-relative min-vh-100 pb-5 px-3" style={{ background: '#f4f6f3', paddingTop: '110px' }}>
      ...
      >
        ...
        <div className="ms-auto d-flex align-items-center gap-2">
          ...
          {isAuthenticated ? (
            ...
            <>
              <Link to="/login" className="btn btn-outline-dark btn-sm px-3 rounded-pill fw-medium">
                Login
              </Link>
              <Link to="/register" className="btn btn-custom btn-sm px-3 rounded-pill fw-medium">
                Register
              </Link>
            </>
          )}
      ...
      <div className="container position-relative" style={{ zIndex: 10 }}>
        ...
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            <h1 className="h3 fw-bold font-josefin m-0" style={{ color: '#2c3524', letterSpacing: '0.05em' }}>Dashboard</h1>
            {isAuthenticated ? (
              <p className="small text-muted mb-0">Welcome back, <span className="fw-semibold text-dark">{user?.username}</span></p>
            ) : (
              <p className="small text-muted mb-0">Welcome, <span className="fw-semibold text-dark">Guest</span> (login to create questions)</p>
            )}
          </div>
          <span 
            className="badge rounded-pill px-3 py-2 text-dark font-monospace" 
            style={{ 
              backgroundColor: isAuthenticated 
                ? (user?.admin ? 'rgba(220, 53, 69, 0.12)' : 'rgba(98, 114, 84, 0.12)') 
                : 'rgba(108, 117, 125, 0.12)', 
              border: isAuthenticated 
                ? (user?.admin ? '1px solid rgba(220, 53, 69, 0.2)' : '1px solid rgba(98, 114, 84, 0.2)') 
                : '1px solid rgba(108, 117, 125, 0.2)'
            }}
          >
            {isAuthenticated ? (user?.admin ? 'ADMIN MODE' : 'STUDENT MODE') : 'GUEST MODE'}
          </span>
        </div>

        {/* Tab 1: QUIZZES TAB */}
        {activeTab === 'quizzes' && (
          <div className="d-flex flex-column overflow-hidden" style={{ maxHeight: 'calc(100vh - 200px)' }}>
            <div className="d-flex justify-content-between align-items-center mb-4">
              <h4 className="fw-bold font-josefin text-uppercase m-0" style={{ color: '#2c3524', letterSpacing: '0.05em' }}>
                Available Quizzes
              </h4>
              <input 
                type="text" 
                  ...
                  {quizzes && quizzes.length > 0 ? (
                        ...
                        <div className="col-md-6" key={quiz._id}>
                          <div className="card quiz-card h-100 p-4 border-0">
                            ...
                            <div className="d-flex gap-2 mt-auto">
                              <button 
                                className="btn btn-custom btn-md flex-grow-1 py-2 rounded-3 fw-medium font-josefin" 
                    ...
                    <div className="col-12">
                      <div className="card text-center p-5 border-0 rounded-4" style={{ backgroundColor: 'var(--natural-color)' }}>
                        <p className="text-muted mb-0">Hiện tại chưa có đề trắc nghiệm nào khớp hoặc được tạo.</p>
                      </div>
                    </div>
                  )}
                </div>
        ...
        {activeTab === 'questions' && (
          isAuthenticated ? (
            /* Logged-in User Question View (Split Screen) */
            <div className="row g-4">
              ...
              <div className="col-lg-7 d-flex flex-column overflow-hidden" style={{ maxHeight: 'calc(100vh - 200px)' }}>
                <div className="d-flex justify-content-between align-items-center mb-4">
                  <h4 className="fw-bold font-josefin m-0" style={{ color: '#2c3524' }}>Questions Bank ({questions?.length || 0})</h4>
                  <input 
                    type="text" 
                    className="form-control form-control-sm rounded-pill px-3" 
                    style={{ maxWidth: '200px' }} 
                        ...
                        .map((q: any) => {
                          ...
                          return (
                            <div className="card p-4 border-0 shadow-sm rounded-4" key={q._id} style={{ background: 'var(--natural-color)' }}>
                              <h5 className="fw-bold h6 mb-3 font-josefin" style={{ color: '#2c3524', lineHeight: '1.4' }}>{q.text}</h5>
            ...
            /* Guest Question View (Full 2-Column Scrollable List, no create form) */
            <div className="d-flex flex-column overflow-hidden" style={{ maxHeight: 'calc(100vh - 200px)' }}>
              <div className="d-flex justify-content-between align-items-center mb-4">
                <h4 className="fw-bold font-josefin text-uppercase m-0" style={{ color: '#2c3524', letterSpacing: '0.05em' }}>
                      ...
                      questions
                        ...
                        .map((q: any) => (
                          <div className="col-md-6" key={q._id}>
                            <div className="card p-4 border-0 shadow-sm rounded-4 h-100 d-flex flex-column justify-content-between" style={{ background: 'var(--natural-color)' }}>
                              <div>
                                <h5 className="fw-bold h6 mb-3 font-josefin" style={{ color: '#2c3524', lineHeight: '1.4' }}>{q.text}</h5>
