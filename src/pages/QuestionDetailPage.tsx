...
export default function QuestionDetailPage() {
  ...
  const { currentQuestion, loading, error } = useSelector((state: any) => state.manager);
    ...
    <div className="position-relative min-vh-100 pb-5 px-3" style={{ background: '#f4f6f3', paddingTop: '110px' }}>
      ...
      >
        <div className="d-flex gap-3">
          ...
          <button className="btn nav-link-custom active btn-link text-decoration-none px-2 py-1" onClick={() => navigate('/dashboard')}>Question Detail</button>
      ...
      <div className="container position-relative" style={{ zIndex: 10 }}>
        {loading ? (
          ...
          <div className="mx-auto" style={{ maxWidth: '650px' }}>
            <div className="card p-5 border-0 shadow-lg rounded-4" style={{ backgroundColor: 'var(--natural-color)' }}>
              <div className="mb-4">
                <span className="badge rounded-pill bg-dark px-3 py-2 text-white font-monospace text-uppercase" style={{ letterSpacing: '0.05em' }}>
                  Question Details
                </span>
              </div>
              
              <h2 className="fw-bold font-josefin mb-4" style={{ color: '#2c3524', lineHeight: '1.4' }}>
                ...
                {currentQuestion.options?.map((opt: string, idx: number) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isCorrect && <span className="badge bg-success ms-auto small rounded-pill px-3 py-1.5">Correct Answer</span>}
                    </li>
                  );
                })}
              </ul>
                ...
                <div className="mb-4">
                  ...
                  <div className="d-flex flex-wrap gap-2">
                    {currentQuestion.keywords.map((kw: string, i: number) => (
                      <span className="badge bg-secondary rounded-pill px-2.5 py-1.5" key={i}>{kw}</span>
                    ))}
                  </div>
                </div>
              )}
